...
const updateTask=async(req,res)=>{
    try{
        ...
        res.json({message:"Task is updated successfully",updatedTask});
    }catch(error){
        res.status(500).json({message:"Server error",error:error.message});
    }
}
...
const updateTaskStatus=async(req,res)=>{
    try{
        ...
        res.json({message:"Task status is updated successfully",task});
    }catch(error){
        res.status(500).json({message:"Server error",error:error.message});
    }
}
...
const updateTaskChecklist=async(req,res)=>{
    try{
        ...
        res.json({message:"Task checklist is updated successfully",task:updatedTask});
    }catch(error){
        res.status(500).json({message:"Server error",error:error.message});
    }
}
...
const getUserDashboardData=async(req,res)=>{
    try{
        ...
        const recentTasks=await Task.find({assignedTo:userId}).sort({createdAt:-1}).limit(10).select("title status dueDate createdAt");
        res.status(200).json({
            statistics:{
                totalTasks,
                pendingTasks,
